...
Master::Master():
  ...
  speed_threshold(0.2)
{
  vel_pub_ = nh_.advertise<Twist>("/NO", 1); 
...
void Master::publishSensor(){
	...
	if ((sensorMsg.angle >= x) && (sensorMsg.angle <= (360-x))){
		if((sensorMsg.angle >= sonartemp.angle + x) || (sensorMsg.angle <= sonartemp.angle - x)) 
			msgBool = true;
	}
	else if(sensorMsg.angle < x){
		if((sensorMsg.angle >= sonartemp.angle + x) || (addAngle(0,sensorMsg.angle) <= addAngle(-10,sonartemp.angle))) 
			msgBool = true;
	}
	else if(sensorMsg.angle > (360-x)){
		if((addAngle(0,sensorMsg.angle) >= addAngle(10,sonartemp.angle)) || (sensorMsg.angle <= sonartemp.angle - x)) 
			msgBool = true;
	}
	else
		msgBool = false;
