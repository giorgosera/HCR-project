  ...
  angular_(0)
{
  ...
  joy_sub_ = nh_.subscribe<Twist>("toSonar", 1, &SonarCheck::joyCallback, this);

  sonar_sub_ = sonar_.subscribe<hcr_vip::sonar_vip>("sonar_vip",1, &SonarCheck::sonarCallback,this); 	
//signal(SIGINT,quit);

}

void SonarCheck::sonarCallback(const hcr_vip::sonar_vip::ConstPtr& sonar){
ok = false;
...
void SonarCheck::checkDistance(float linear){
...
void SonarCheck::joyCallback(const Twist::ConstPtr& joy)
{
